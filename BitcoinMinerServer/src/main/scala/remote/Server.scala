...
object HelloRemote {
	def main(args : Array[String]) {
		...
		val masterActor = system.actorOf(Props(new Master(nrOfWorkers, nrOfMessages, nrOfElements,compString, listener)), name = "MasterActor")
	...
	def calculateBtc(start: Int, nrOfElements: Int,compString: String): java.util.ArrayList[BitCoin] = {
		...
		for ( k <- start until (start + nrOfElements)) {
			...
			for (i <- 0 until byteData.length) {
				sb.append(java.lang.Integer.toString((byteData(i) & 0xff) + 0x100, 16).substring(1))
			}
